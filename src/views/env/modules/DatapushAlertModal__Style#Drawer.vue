    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="告警扩展信息" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="['properties']" placeholder="请输入告警扩展信息"></a-input>
        </a-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/cgauto/datapushAlert/add",
          edit: "/cgauto/datapushAlert/edit",
        }
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'orgCode','alertId','name','severity','description','entityName','entityAddr','firstTime','lastTime','properties','ciId','proStatus','orderNo'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'orgCode','alertId','name','severity','description','entityName','entityAddr','firstTime','lastTime','properties','ciId','proStatus','orderNo'))
      }
