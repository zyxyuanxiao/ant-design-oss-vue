  ...
  <div style="background: #ffffff">
    ...
    <a-card title="意见信息" :bodyStyle="{padding:'0 20px'}" size="default" style="margin-top:20px">
      <a-list itemLayout="vertical">
        <template v-for="(item) in resultObj.bpmLogList">
          <a-list-item :key="item.taskName">
            ...
            <template v-for="(file) in item.bpmFiles">
              <div class="ant-upload-list ant-upload-list-text" :key="file.filePath">
            ...
            <a-col :span="24">
              ...
              <span :hidden="this.model.processModel!==2">
                ...
                <template v-for="(item) in resultObj.transitionList">
                  <a-checkbox :checked="true" :value="item.nextnode" :key="item.nextnode">{{ item.Transition }}</a-checkbox>
                </template>
                <!-- </a-checkbox-group>-->
              </span>
              <span :hidden="this.model.processModel!==3" v-if="resultObj.histListSize>0">
                <a-select v-model="model.rejectModelNode" :getPopupContainer = "(target) => target.parentNode" style="width:150px">
                  <template v-for="(item) in resultObj.histListNode">
                    <a-select-option v-if="item.NAME_ != resultObj.taskName " :value="item.TASK_DEF_KEY_" :key="item.TASK_DEF_KEY_">{{ item.NAME_ }}</a-select-option>
                  </template>
                </a-select>
              </span>
            </a-col>
