<template>
  <div>
    <div style="margin-bottom: 20px;height: 400px;overflow: hidden;overflow-y: auto;overflow-x: auto;">
      ...
      <map name="planetmap">
        <template v-for="(item, index) in nodePositionInfo.positionList">
         <area shape="rect" :coords="item.coords" title="Venus" @mouseover="showNodeInfo(nodePositionInfo.hisTasks,item.id)" :key="index">
        </template>
      </map>
    </div>
    <a-card title="流程历史跟踪">
